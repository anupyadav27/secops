...
def scan_file(py_file, rules):
    ...
    for rule in rules:
        ...
        all_findings.extend(rule.check(ast_tree, py_file))
...
if __name__ == "__main__":
    ...
    # Parse file and print Raise nodes and their parent types
    # ast_tree = parse_python_file(py_file)
    # def print_raise_nodes(ast_tree, visited=None):
    #     if visited is None:
    #         visited = set()
    ...
    #             return
    #         visited.add(node_id)
    #         if ast_tree.get('node_type') == 'Raise':
    #             parent = ast_tree.get('parent')
    #             exc = ast_tree.get('exc', 'MISSING')
    #             print(f"Raise node at line {ast_tree.get('lineno')}, exc: {exc}, parent type: {parent.get('node_type') if parent else None}")
    #             print(f"  Full node: {json.dumps({k: v for k, v in ast_tree.items() if k != 'parent'}, indent=2)}")
    #         for key, value in ast_tree.items():
    #             if key == 'parent':
    #                 continue
    #             print_raise_nodes(value, visited)
    #     elif isinstance(ast_tree, list):
    #         for item in ast_tree:
    #             print_raise_nodes(item, visited)
    # print('--- AST Raise Node Details ---')
    # print_raise_nodes(ast_tree['module'])
    # print('--- End AST Print ---')
